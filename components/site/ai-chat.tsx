...
function AIConversation({ seller = false }: { seller?: boolean }) {
    ...
    <>
      <section className="ac-conversation" aria-label={seller ? 'Seller AI support' : 'Auronix AI support'}>
        <header className="ac-conversation-header">
          <div className="ac-assistant-identity"><ChatBrandMark className="h-11 w-11"/><div><strong>Auronix AI</strong><span>{seller ? 'Seller guidance' : 'Commerce support'} · AI assistant</span></div></div>
        ...
        <div className="ac-chat-scroll" ref={scrollRef} role="log" aria-label="Conversation" aria-live="polite" aria-relevant="additions">
          ...
          {loading&&<article className="ac-message" data-role="assistant"><div className="ac-message-label">Auronix AI</div>{visibleAnswer?<div className="ac-message-content">{renderMarkdown(visibleAnswer,true)}</div>:<div className="ac-chat-thinking" role="status"><Spinner className="w-5 h-5"/> Looking into your question…</div>}</article>}
...
export function AIChat({ mode = 'launcher', seller = false }: { mode?: 'launcher'|'page'; seller?: boolean }) {
 return mode==='page' ? <AIConversation seller={seller}/> : <Link href="/support/chat" className="ac-support-launcher" aria-label="Open Auronix AI chat"><MessageCircle size={22}/><span>Get help</span></Link>;
}
